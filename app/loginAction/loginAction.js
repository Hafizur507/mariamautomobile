...
//   if (user) {
//     return throw new Error('user already register');
//   }
...
export async function RegisterForm(prevState, formData) {
  const email = formData.get('email');
  const password = formData.get('password');

  // ১. ইউজার ইতিমধ্যে আছে কিনা চেক
  const user = await User.findOne({ email });
  if (user) {
    return { error: 'An account has already been created with this email!' };
  }

  // ২. পাসওয়ার্ড হ্যশিং ও ডাটাবেজে ইউজার তৈরি
  try {
    const hashedPassword = await bcrypt.hash(password, 10);
    await User.create({ email, password: hashedPassword });
  } catch (err) {
    return { error: 'Somthing want worng' };
  }
...
export async function ReviewSubmit(formdata) {
  ...
  if (costomerimage && costomerimage instanceof File && costomerimage.size > 0) {
